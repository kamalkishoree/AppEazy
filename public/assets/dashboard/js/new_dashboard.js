$(document).ready(function () {
    ...
    function getDashboardData(dashboard_filter_url) {
        console.log(dashboard_filter_url);
        ...
        $.getJSON(
            ...
            function (response) {
                ...
                if (response.status == "Success") {
                    ...
                    $("#notification_counts").html(
                        response.data.orderNotificationCnt
                    );
                    $("#total_revenue").html(
                        response.data.currencySymbol +
                            response.data.total_revenue
                    );
                    $("#total_sold_products").html(
                        response.data.total_sold_products
                    );
                    ...
                    $("#revenueCurrentWeek").html(
                        response.data.currencySymbol +
                            response.data.revenueCurrentWeek
                    );
                    $("#revenueLastWeek").html(
                        response.data.currencySymbol +
                            response.data.revenueLastWeek
                    );
                    if (response.data.customers_increase != "") {
                        $("#customers_change").html("");
                        $("#customers_change").append(
                            '<span class="text-success me-2"><i class="mdi mdi-arrow-up-bold"></i>' +
                                response.data.customers_increase +
                        ...
                        $("#customers_change").append(
                            '<span class="text-danger me-2"><i class="mdi mdi-arrow-down-bold"></i>' +
                                response.data.customers_decrease +
                                '%</span><span class="text-nowrap">Since last month</span>'
                        );
                    } else {
                        $("#customers_change").html("");
                        $("#customers_change").append(
                            '<span class="text-danger me-2">0%</span><span class="text-nowrap">Since last month</span>'
                        );
                    }
                    if (response.data.orders_increase != "") {
                        $("#orders_change").html("");
                        $("#orders_change").append(
                            '<span class="text-success me-2"><i class="mdi mdi-arrow-up-bold"></i>' +
                                response.data.orders_increase +
                                '%</span><span class="text-nowrap">Since last month</span>'
                        );
                    ...
                    if (response.data.revenue_increase != "") {
                        ...
                        $("#revenue_change").append(
                            '<span class="text-success me-2"><i class="mdi mdi-arrow-up-bold"></i>' +
                                response.data.revenue_increase +
                                '%</span><span class="text-nowrap">Since last month</span>'
                        );
                        ...
                        $("#revenue_change").append(
                            '<span class="text-danger me-2"><i class="mdi mdi-arrow-down-bold"></i>' +
                                response.data.revenue_decrease +
                                '%</span><span class="text-nowrap">Since last month</span>'
                        );
                    } else {
                        $("#revenue_change").html("");
                    ...
                    if (response.data.products_increase != "") {
                        ...
                        $("#products_change").append(
                            '<span class="text-success me-2"><i class="mdi mdi-arrow-up-bold"></i>' +
                                response.data.products_increase +
                                '%</span><span class="text-nowrap">Since last month</span>'
                        );
                        ...
                        $("#products_change").append(
                            '<span class="text-danger me-2"><i class="mdi mdi-arrow-down-bold"></i>' +
                                response.data.products_decrease +
                                '%</span><span class="text-nowrap">Since last month</span>'
                        );
                    } else {
                        $("#products_change").html("");
                    ...
                    updateRevenue(
                        response.data.monthwise_revenue,
                        response.data.currencySymbol
                    );
                    updateRevenueLineChart(
                        response.data.currentweek_revenue_daywise,
                        response.data.previousweek_revenue_daywise,
                        response.data.currencySymbol
                    );
                    ...
                    if (response.data.locationwise_revenue) {
                        ...
                        if (response.data.locationwise_revenue != "") {
                            ...
                            for (const property in arr) {
                                ...
                                var orderCount =
                                    response.data.currentyear_ordercount;
                                var percent = Math.round(
                                    (data.addressCount / orderCount) * 100
                                );
                                var html =
                                    '<span id="' +
                                    data.city +
                                    ...
                                    data.city +
                                    '</h5><div class="progress-w-percent"><span class="progress-value fw-bold">' +
                                    Math.round(data.sum) +
                                    "</span><div class='progress progress-sm'><div class='progress-bar' role='progressbar' style='width:" +
                                    percent +
                                    "%;' aria-valuenow='72' aria-valuemin='0' aria-valuemax='100'></div></div></div></span>";
                                $("#revenue_locations").append(html);
                            }
                        } else {
                            $("#revenue_locations").append(
                                '<h5 class="mb-1 mt-0 fw-normal text-center">No data found</h5>'
                            );
                        }
                    }
                }
    ...
    function updateRevenue(newrevenue, currency) {
        ...
        var chart = new ApexCharts(
            document.querySelector("#revenue-bar-chart"),
            options
        );
        chart.render();
    }
    ...
    function updateRevenueLineChart(current_data, previous_data, currency) {
        console.log("line chart", current_data, previous_data, currency);
        ...
        var options = {
            ...
            series: [
                { name: "Current Week", data: current_data },
                { name: "Previous Week", data: previous_data },
            ],
            colors: colors,
            zoom: { enabled: !1 },
            legend: { show: !1 },
