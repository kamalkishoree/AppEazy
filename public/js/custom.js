...
window.easyZoomInitialize = function easyZoomInitialize() {
    ...
    // gallery-thumbs
    let productCarouselThumbsItemWith = thumbs
        .find(".swiper-slide")
        .outerWidth();
...
window.initializeSlider = function initializeSlider() {
    ...
    $(".brand-slider").slick({
        ...
        slidesToScroll: 3,
        responsive: [
            { breakpoint: 1367, settings: { slidesToScroll: 2, infinite: !0 } },
        ...
        success: function (response) {
            if (response.status == "Success") {
                $.ajax({
                    type: "get",
                    dataType: "json",
                    url: subscription_payment_options_url.replace(
                        ":id",
                        sub_id
                    ),
                    success: function (response) {
                        if (response.status == "Success") {
                            ...
                            $(
                                "#subscription_payment #subscription_frequency"
                            ).html(response.sub_plan.frequency);
                            $("#subscription_payment #features_list").html(
                                response.sub_plan.features
                            );
                            $("#subscription_payment #subscription_id").val(
                                sub_id
                            );
                            ...
                            $(
                                "#subscription_payment #subscription_payment_methods"
                            ).html("");
                            ...
                            if (response.payment_options == "") {
                                $(
                                    "#subscription_payment .subscription_confirm_btn"
                                ).hide();
                            }
                            $("#subscription_payment").modal("show");
                            if (response.sub_plan.type_id == 2) {
                                var form = document.getElementById(
                                    "meal-subscription-form"
                                );
                                ...
                                var serializedField =
                                    document.createElement("input"); // Create a new input element
...
$(document).delegate(".subscription_confirm_btn", "click", function () {
    ...
    var selected_option = $(
        "input[name='subscription_payment_method']:checked"
    );
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status == "success") {
                ...
                if (
                    $("#tbody_" + vendor_id + " > vendor_products_tr").length ==
                    0
                ) {
                ...
                if ($("#show_plus_minus" + cartproduct_id).length != 0) {
                    if (
                        ...
                        $(".addon_variant_quantity_" + cartproduct_id)
                            ...
                            .remove();
                        var total_qty = $(
                            '.add_vendor_product[data-product_id="' +
                                product_id +
                                '"]'
                        )
                            .next()
    ...
    if ($(this).val()) {
        ...
        if (is_service_product_price_from_dispatch_forOnDemand == 1) {
            ...
            Swal.fire({
                ...
                text: _language.getLanString(
                    "Sorry ! address can not be change!"
                ),
            });
            return false;
        }
...
$(document).on("click", "#order_placed_btn", async function () {
    ...
    $(".validate_prescription").each(function () {
        var cart_product_prescription = $(this).data(
            "cart_product_prescription"
        );
    ...
    if (task_type == "schedule") {
        if (slot) {
            ...
            var schedule_dtck = toTimestamp(
                schedule_dt + " " + slot.split(" - ", 1)
            );
        ...
        if (business_type == "laundry" && scheduling_with_slots == 1) {
            ...
            var schedule_dt_dropoff =
                schedule_dt_dropoff + schedule_dt_dropoff_time;
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.status == "passbase_submitted") {
                    ...
                    $.each(
                        response.presciptionProducts,
                        function (key, product_id) {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            if (response.status == "Success") {
                                ...
                                let payment_method_tab_pane_template =
                                    _.template(
                                        $(
                                ...
                                if (advanceCartTotalPayableAmount == 1) {
                                    var amtHTML =
                                        ...
                                        "</span>";
                                    $(
                                        "#proceed_to_pay_modal #pay-billLabel"
                                    ).html(amtHTML);
                                } else {
...
function stripePaymentMethodHandler(result) {
    ...
    if (path.indexOf("cart") !== -1) {
        ...
        paymentAjaxData.send_card_is_delivery = $(
        ...
        success: function (response) {
            if (response.status == "Success") {
                ...
                if (path.indexOf("cart") !== -1) {
                    ...
                    $("#topup_wallet_btn, .topup_wallet_confirm").removeAttr(
                        "disabled"
                    );
                } else if (
                    cabbookingwallet != undefined &&
...
$(document).on("click", ".proceed_to_pay", function () {
    ...
    $("#order_placed_btn, .proceed_to_pay").attr("disabled", true);
    var delivery_type = $("input:radio.delivery-fee:checked").attr(
        "data-dcode"
    );
    var other_taxes_string = "";
    if ($("#other_taxes_string").val() != null) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.status == "Success") {
                ...
                $("#topup_wallet_btn, .topup_wallet_confirm").attr(
                    "disabled",
                    false
                );
            }
...
$(document).on("click", ".topup_wallet_confirm", function () {
    ...
    if (
        ...
        $("#wallet_payment_methods_error").html(
            payment_method_required_error_msg
        );
        return false;
    } else {
        $("#wallet_payment_methods_error").html("");
                    ...
                    if (mapAddress.long_name != "") {
                        ...
                        if (mapAddress.types[0] == "locality") {
                            document.getElementById("city").value =
                                mapAddress.long_name;
                        }
                        if (
                            mapAddress.types[0] == "administrative_area_level_1"
                        ) {
                            document.getElementById("state").value =
                                mapAddress.long_name;
                            document.getElementById("state_code").value =
                                mapAddress.short_name;
                        }
                    ...
                    if (cart_details.products.length > 0) {
                        ...
                        if (response.cart_details.totalQuantity > 0) {
                            ...
                            if (
                                ...
                                $("#edit_order_schedule_datetime").val() !=
                                    "" &&
                                typeof $("#edit_order_schedule_slot").val() !=
                                    "undefined" &&
                                $("#edit_order_schedule_slot").val() != ""
                            ) {
                                var edit_order_schedule_datetime = $(
                                ...
                                schedule_datetime =
                                    edit_order_schedule_datetime.split(" ")[0];
                            } else {
                                schedule_datetime = $(
                                    "#edit_order_schedule_datetime"
                                ).val();
                        ...
                        if ($("#header_cart_template_ondemand").length != 0) {
                            ...
                            $("#header_cart_main_ul_ondemand").removeClass(
                                "d-none"
                            );

                            $("#header_cart_main_ul_ondemand").html(
                                header_cart_template_ondemand(
                                    CartTemplateOndemandData
                                )
                            );
                ...
                if (response.cart_details.length != 0) {
                    if (response.cart_details.products.length != 0) {
                        ...
                        if ($("#cart_main_page").length != 0) {
                            ...
                            let cart_template = _.template(
                                $("#cart_template").html()
                            );
                            $("#cart_table").append(
                                cart_template(extendedData)
                            );
    ...
    ajaxCall = $.ajax({
        ...
        success: function (response) {
            if (response.status == "error") {
                ...
                $("#product_total_amount_" + cartproduct_id).html(
                    "$" + latest_price
                );
                // return false;
                cartHeader();
...
function updateCartProductStatus(cartproduct_id, is_cart_checked) {
    ajaxCall = $.ajax({
        ...
        data: {
            ...
            is_cart_checked: is_cart_checked,
        },
        success: function (response) {
            if (response.status == "error") {
                Swal.fire({
...
function setTipAmount(tip, amount_payable, currency) {
    ...
    if (tip != "custom") {
        ...
        if (
            isNaN(wallet_amount_used_fixed) ||
            wallet_amount_used_fixed == null
        ) {
            wallet_amount_used_fixed = 0;
        }
        amount_payable =
            parseFloat(amount_payable) + parseFloat(tip) + other_taxes;
        $("#cart_tip_amount").val(
            parseFloat(tip).toFixed(parseInt(digit_count))
        );
        $("#cart_total_payable_amount").html(
        ...
        $("#cart_total_payable_amount").text(
            currency +
                parseFloat(amount_payable + other_taxes).toFixed(
                    parseInt(digit_count)
                )
        );
...
$(document).on("keyup", "#custom_tip_amount", function () {
    ...
    if (wallet_amount_available > 0) {
        ...
        payable_amount =
            ...
            (total_subscription_discount + loyalty_amount);
        $("#cart_total_payable_amount").html(
            currency + payable_amount.toFixed(parseInt(digit_count))
        );
        $("input[name='cart_total_payable_amount']").val(
...
async function showRemoveCart(modelText) {
    ...
    if (
        typeof service_start_time != "undefined" &&
        service_start_time !== null
    ) {
        var data_service_start_time = service_start_time;
    }
    if (
        typeof total_booking_time != "undefined" &&
        total_booking_time !== null
    ) {
...
function addToCart() {
    ...
    if ($("#is_recurring_booking").length > 0) {
        ...
        if (product_id == OrderStorage.getStorage("cartFirstProductId")) {
            var message = _language.getLanString(
    ...
    if ($("#date_input").val() == "") {
        Swal.fire({
            text: _language.getLanString(
                "Please select delivery date to continue"
            ),
            icon: "warning",
            button: "OK",
        });
    ...
    $(".productAddonSetOptions").each(function (index) {
        ...
        if (
            min_select > 0 &&
            $(this).find(".productDetailAddonOption:checked").length <
                min_select
        ) {
            Swal.fire({
                // title: "Warning!",
    ...
    if (!breakOut) {
        ...
        if (sVendorResponse.status == "Success") {
            ...
            if (
                ...
                (OrderStorage.getStorage("cartProductCount") > 0 &&
                    addLongTerm == 1)
            ) {
        ...
        success: function (response) {
            if (response.status == "Success") {
                ...
                $("#estimate_product_addon_modal .modal-content").append(
                    estimate_addon_template({
                        estimateAddOnData: response.data,
                    })
                );
    ...
    ajaxCall = $.ajax({
        ...
        success: function (response) {
            if (response.status == "Success") {
                ...
                $("#repeat_item_modal")
                    .find(".curr_product_id")
                    .val(product_id);
                $("#repeat_item_modal")
                    .find(".curr_product_has_addons")
                    .val(addon);
            }
        },
        error: function (response) {
    ...
    ajaxCall = $.ajax({
        ...
        success: function (response) {
            if (response.status == "error") {
                ...
                $("#quantity_ondemand_" + cartproduct_id).val(
                    response.quantity
                );
            } else {
...
$(document).delegate(".product_addon_option", "click", function () {
    ...
    if (
        addonSet.find(".product_addon_option:checked").length > addon_maxlimit
    ) {
        this.checked = false;
    }
    ...
    if (that.hasClass("add_vendor_addon_product")) {
        ...
        that.parents(".modal")
            ...
            .each(function (index) {
                ...
                if (
                    min_select > 0 &&
                    $(this).find(".product_addon_option:checked").length <
                        min_select
                ) {
                    success_error_alert(
                        "error",
            ...
            if (response.status == "success") {
                ...
                if (that.hasClass("add_vendor_addon_product")) {
                    ...
                    $(that)
                        ...
                        .attr(
                            "id",
                            "quantity_ondemand_" + response.cart_product_id
                        );
                    $(that)
                        .next()
                    ...
                    $(that).attr(
                        "id",
                        "add_button_href" + response.cart_product_id
                    );
                    $(that).hide();
                    $(that).next().show();
        ...
        if (schedule_type != "ProductDateTime") {
            if (schedule_type == "date") {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status == "Success") {
                            $("#vendor_schedule_slot_" + vendor_id).html(
                                response.data
                            );
                        } else {
                            success_error_alert(
...
function getTimeSlots(
    ...
    $.ajax({
        ...
        success: function (response) {
            var booking_time_slick = $(
                "#show-all-time-slots" + cart_product_id
            ).find(".booking-time");
            ...
            $(
                "#show-all-time-slots" + cart_product_id + " .booking-time"
            ).slick({
                dots: !1,
                infinite: !0,
                speed: 300,
    ...
    switch (action) {
        ...
        case "4":
            stripe.createSource(card).then(function (result) {
                if (result.error) {
                    $("#stripe_card_error").html(result.error.message);
                    $("#order_placed_btn, .proceed_to_pay").attr(
                        "disabled",
                        false
                    );
                } else {
