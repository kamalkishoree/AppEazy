...
export default function DashBoard({ route, navigation }) {
  ...
  useFocusEffect(
    React.useCallback(() => {
      if (selectedOption != 2 && (!!userData?.auth_token||!!userData?.access_token)) {
        getTasks();
      } else {
  ...
  const getCurrentPosition = () => {
    return navigator.geolocation.default.getCurrentPosition(
      position => {
        ...
        fetchgentLogs(position.coords.latitude, position.coords.longitude, position.coords.heading);
  ...
  const _onRidePriceIncerimentDecrimentPrice = (type, bidData) => {
    let incremtalVal =  getBundleId() == appIds?.groupy ? 1 : 10;
    if (type == 'minus') {
      return Number(bidData?.selectedPriceForBid) - incremtalVal
    } else {
      return Number(bidData?.selectedPriceForBid) + incremtalVal;
    }
  };
