...
export default function Filter({route, navigation}) {
  ...
  const applyFilters = () => {
    ...
    //All Brand Ids
    console.log(allFilterData,'allFilterDataallFilterData');

    if (showBrands && !!allFilterData.length) {
      ...
      let ids = allFilterData
      .filter((i) => i?.id == -1) 
      brandsIds = !!ids.length ? ids[0].value.filter((itm) => itm?.value?.selected == true)
        .map((j) => {
          return j.id;
        }) : [];
