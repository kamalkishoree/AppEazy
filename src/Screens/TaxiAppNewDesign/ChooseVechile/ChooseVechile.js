...
function ChooseVechile({ navigation, route }) {
    ...
    const _finalPayment = (data) => {
        ...
        let priceToPay =  selectedCarOption?.total_tags_price
        ...
        if (isEmpty(selectedPayment) && (priceToPay != 0) ) {
    ...
    const _confirmAndPay = () => {
        ...
        data['remaing_amount'] = Number(selectedCarOption?.remaining_amount) - (updatedAmount ? updatedAmount : 0);
        data['wallet_amount_used'] = Number(selectedCarOption?.wallet_amount_used) - ((Number(selectedCarOption?.remaining_amount) == 0 && updatedAmount) ? updatedAmount : 0);
        data['tasks'] = paramData?.tasks;
        data['images_array'] = uploadImages;
        data["unique_id"] = uID;
        data["send_to_all"] = (!!issendtoall ? 1 : 0)
            ...
            <BottomSheet
                ...
                <BottomSheetScrollView
                    ...
                    <View
                        ...
                        {!!profile?.preferences?.is_particular_driver && cabBookingType === "Booking" && !showPaymentModal &&
                            <TouchableOpacity onPress={() => { setIssendtoall(!issendtoall) }} style={{ marginHorizontal: moderateScale(18), marginBottom: moderateScaleVertical(8), flexDirection: 'row', alignItems: 'center' }}>
                                <Image style={{ marginRight: moderateScale(12) }} source={!!issendtoall ? imagePath.checkBox2Active : imagePath.checkBox2InActive} />
                                <Text style={{ color: isDarkMode ? MyDarkTheme.colors.text : colors.textGreyOpcaity7 }}>Notify other drivers if preferred driver not available</Text>
