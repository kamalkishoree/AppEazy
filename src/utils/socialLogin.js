  ...
  return await new Promise(async (resolve, reject) => {
    ...
    if (checkAppleSupport) {
      ...
      if (credentialState === appleAuth.State.AUTHORIZED) {
        ...
        const storedAppleUser = await AsyncStorage.getItem('appleUser');
        const storedUser = !!storedAppleUser ? JSON.parse(storedAppleUser) : null;

        console.log(storedAppleUser,'storedAppleUser 111');
        console.log(appleAuthRequestResponse,'storedAppleUser 22');
        if (!!storedUser) {
          // Use the stored user information on subsequent logins
          resolve(storedUser);
        } else {
          // On the first login, store the user information
